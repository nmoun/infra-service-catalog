...
The aim is to have a single entry point for operators to provision new demands, whether it concerns the creation of a resource, an update or its decommissioning.
...
- **plan_callback.yaml**:
  ...
  - purpose: this workflow updates the request status depending on whether the plan succeeded or failed, or if no changes are detected by terraform. In case of success, the plan is shown within the request, and instructions to launch the apply phase are added to the request.
- **launch_apply.yaml**:
  ...
  - purpose: this workflow orders the launch of the *terraform apply*, based on the request content and the specific plan run id for which we want *terraform apply* to be executed.
- **apply_callback**:
